...
run_configs_pwd = "/1 DRL/E2E-MAPPO-for-MT-FJSP-main/config_run.json"
...
test_configs_pwd = "/1 DRL/E2E-MAPPO-for-MT-FJSP-main/tester/config_test.json"
...
parser.add_argument('--weight_carbon', type=float, default=0.4, help='instance')
parser.add_argument('--weight_tou', type=float, default=0.2, help='instance')

# TODO zzo 碳排放与分时电价常数
parser.add_argument('--carbon_grid', type=float, default=0.6747, help='机器消耗电网电能的碳排放因子 (kgCO2/kWh)')
parser.add_argument('--carbon_agv', type=float, default=0.6747, help='AGV消耗电能的碳排放因子 (kgCO2/kWh)')
# 格式为列表嵌套元组: (开始小时, 结束小时, 电价 元/kWh)
# 高峰(1.20元): 10-12, 14-19
# 平段(0.85元): 08-10, 12-14, 19-24
# 低谷(0.35元): 00-08
# 注意：24小时必须无缝衔接，不能有遗漏的时间点！
tou_price_table = [
        (0,  8,  0.35),   # 低谷
        (8,  10, 0.85),   # 平段
        (10, 12, 1.20),   # 高峰
...
parser.add_argument('--tou_price_table', type=list, default=tou_price_table, help='24小时分时电价区间表')
# TODO zzo 碳排放与分时电价常数

#parser.add_argument('--price_peak', type=float, default=1.2757, help='峰期电价 (元/kWh)')
#parser.add_argument('--price_flat', type=float, default=0.7653, help='平期电价 (元/kWh)')
#parser.add_argument('--price_valley', type=float, default=0.3551, help='谷期电价 (元/kWh)')
...
parser.add_argument('--log_to_wandb', '-w', type=bool, default=False)  # 是否使用wandb
...
parser.add_argument('--trained_model_pth', type=str, default='/1 DRL/E2E-MAPPO-for-MT-FJSP-main/trained_model/', help='paload_pth_parametersth')
