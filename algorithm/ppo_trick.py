...
class RewardScaling:
    def __init__(self, shape, gamma):
        ...
        self.running_ms = RunningMeanStd(shape=self.shape) # 这是一个统计工具，用来实时计算均值和标准差
        self.R = np.zeros(self.shape) # R 是“折扣累计回报”，记录当前这一局游戏的积累值

    def __call__(self, x):
        # 1. 更新累计折扣回报 R
        # 公式：R_new = R_old * gamma + r_immediate
        self.R = self.gamma * self.R + x
        # 2. 把累计回报 R 喂给记账员，让它更新统计数据（计算新的标准差 std）
        self.running_ms.update(self.R)
        # 3. 缩放即时奖励 x
        # 核心公式：x_scaled = x / (std + epsilon)
