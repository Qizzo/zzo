...
def validate_cost_gcn_jointActor_GAT(ppo, gantt_flag, data, data_index, data_type, greedy=True, args=None):
    ...
    while True:
        ...
        if o_done:
            ...
            """至今为止：一定要在done之后reset之前读取：全选完的当前this step的cost"""
            mk = va_env.makespan_previous_step  # 上次的makespan
            pt = va_env.total_e1_previous_step  # 【修改点 1】：千万不能除以 n_total_task，必须保持物理能耗总量的绝对值！
            transT = va_env.trans_t_previous_step  # 运输时间（原代码代指运输能耗）
            idleT = va_env.idle_t_previous_step  # 机器空闲时间之和

            # 【修改点 2】：把底层跑出的真实分时电费拿出来
            tou_cost = va_env.tou_cost_previous_step

            # 返回基础四指标用于外层打印和算 MIP 误差
            Final_4cost = [mk, pt, transT, idleT]

            # ==========================================================
            # ZZO ===== 严格对齐：验证集真实打分机制 (决定哪个模型被保存) =====
            # ==========================================================
            w1 = configs['weight_mk']
            w2 = configs['weight_carbon']
            w3 = configs['weight_tou']

            # 取出碳排因子 (configs 里存了从 parameter.py 传过来的所有参数)
            carbon_grid = configs['carbon_grid']
            carbon_agv = configs['carbon_agv']

            # 计算真实的机器碳排与物流碳排
            m_carbon = (pt + idleT) * carbon_grid
            agv_carbon = transT * carbon_agv

            # 全新的 3 权重目标打分！只有这三个指标真正平衡得最好的模型，才会被判定为最优！
            Objective = w1 * mk + w2 * (m_carbon + agv_carbon) + w3 * tou_cost
            # ==========================================================

            if gantt_flag:  # validate的画图版本
                # ===================== 终极核查：所有工序到底排上没有？ =====================
                # 遍历所有 Job (0 到 5)
                for j_id in range(n_job):
                    print(f"--- Job {j_id} ---")
                    # 找出属于这个 Job 的所有节点，并按先后顺序排序
                    job_nodes = [n for n, d in va_env.G.nodes(data=True) if d.get('job') == j_id]
                    job_nodes.sort()

                    for i, n_id in enumerate(job_nodes):
                        d = va_env.G.nodes[n_id]
                        status = "✅ 已排上" if d.get('scheduled') else "❌ 没排上!!"
                        m_id = d.get('machine')
                        st = d.get('start_time')
                        ft = d.get('finish_time')

                        # 格式化打印
                        st_str = f"{st:.1f}" if st is not None else "None"
                        ft_str = f"{ft:.1f}" if ft is not None else "None"
                        print(
                            f"  第 {i + 1} 道工序 (节点ID {n_id:2d}): 状态 = {status}, 分配机器 = {m_id:2d}, 开始 = {st_str:>5}, 结束 = {ft_str:>5}")
                print("=" * 64 + "\n")
                # =====================================================================
                va_env.render()
                Logger.log("Evaluation/done", f"Objective={Objective}, mk={mk}, m_carbon={m_carbon}, agv_carbon={agv_carbon}, tou_cost={tou_cost}", print_true=1)
...
def esa_validate_cost_gcn_jointActor_GAT(ppo, gantt_flag, data, data_index, data_type, greedy=True, args=None):
    ...
    while True:
        ...
        if o_done:
            ...
            # ==========================================================
            """至今为止：一定要在done之后reset之前读取：全选完的当前this step的cost"""
            mk = va_env.makespan_previous_step
            pt = va_env.total_e1_previous_step
            transT = va_env.trans_t_previous_step
            idleT = va_env.idle_t_previous_step
            ...
            m_carbon = (pt + idleT) * carbon_grid
            agv_carbon = transT * carbon_agv
            Final_4cost = [mk, m_carbon, agv_carbon, tou_cost]
            # 全新的 3 权重目标打分！只有这三个指标真正平衡得最好的模型，才会被判定为最优！
            Objective =  w1 * mk + w2 * (m_carbon + agv_carbon) +  w3 * tou_cost
            ...
            if gantt_flag:  # validate的画图版本
                ...
                Logger.log("Evaluation/done",
                           f"Objective={Objective}, mk={mk}, m_carbon={m_carbon}, agv_carbon={agv_carbon}, tou={tou_cost}",
                           print_true=1)  # eval的结果
                # ===================================================================
