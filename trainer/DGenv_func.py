...
# TODO zzo 新加时间切分函数
def calculate_tou_cost_by_slicing(start_time, end_time, power, tou_table):
    """
    通过时间切片法计算分时电费成本 (TOU Cost)。
    已设定：t为分钟 (1/60小时)，t=0 对应凌晨 00:00。
    """
    time_unit_hours = 1.0 / 60.0
    abs_start = start_time * time_unit_hours
    abs_end = end_time * time_unit_hours
    total_cost = 0.0
    current_time = abs_start
    while current_time < abs_end:
        time_of_day = current_time % 24.0
        current_period_end = 24.0
        current_price = 0.0
        # 查表确定当前电价区间
        for (p_start, p_end, price) in tou_table:
            if p_start <= time_of_day < p_end:
                current_period_end = p_end
                current_price = price
                break
        # 计算在当前区间能够存活的时间
        time_left_in_task = abs_end - current_time
        time_left_in_period = current_period_end - time_of_day
        step_time_hours = min(time_left_in_task, time_left_in_period)
        # 累加电费：功率(kW) * 时长(h) * 电价(元/kWh)
        total_cost += power * step_time_hours * current_price
        # 推进时间
        current_time += step_time_hours
    return total_cost
